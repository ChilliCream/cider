...
| `LoadImagesAsync` | `image load -i <tmp>.tar` (`Images.cs:451`) | one-shot, `PullTimeout` | stdout `Loaded image: <ref>` echo read unconditionally as the primary source; **before/after `ListImagesAsync` diff** kept only as a secondary, unioned-in source (`918daf4`) | The CLI writes the loaded reference to stdout (`container image load -i <tar>`) — it does not go unreported |
