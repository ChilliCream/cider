...
| **BuildKit context/export transport is paced to 8 MiB/s by default** | All BuildKit bytes (context uploads, image tar exports, status streams) travel over `container exec -i buildkit buildctl dial-stdio`, throttled by a token-bucket pacer (`TokenBucketPacer.DefaultBytesPerSecond`) so a large transfer cannot overrun buildkitd's HTTP/2 receive window and wedge the pipe. Measured on this evidence pass: a 200 MiB build context takes ~25s end to end (~8 MiB/s, exactly the pacer's steady rate) with the pacer at its shipped default, and a 585 MB `--output type=tar` image export takes ~5s (~120 MiB/s) once the pacer is raised past that image's size — in both cases with zero stalls or link-recovery events logged. That second number is the important one: it shows the underlying exec pipe itself sustains well over 100 MiB/s with no backpressure problems at these sizes, so the shipped 8 MiB/s is a conservative constant chosen before any large-context measurement existed, not evidence that the exec-based transport is the bottleneck. A non-exec bulk-transport path was evaluated on this evidence and is **not** implemented — the numbers do not justify it; retuning the pacer's default rate is a smaller, separate follow-up |
