...
| **BuildKit context uploads are paced to 8 MiB/s by default** | Only the bytes the daemon sends *to* buildkitd — build context uploads via FileSync/DiffCopy — pass through a token-bucket pacer (`TokenBucketPacer.DefaultBytesPerSecond`) on the exec pipe (`container exec -i buildkit buildctl dial-stdio`), so a large upload cannot overrun buildkitd's HTTP/2 receive window and wedge it. Bytes buildkitd sends back — image tar exports via FileSend, and the Control/Status streams — are not paced at all; that direction is bounded only by the connection's 256 KiB initial HTTP/2 stream window (`BuilderConnection.InitialStreamWindowBytes`). At the shipped default a 200 MiB build context takes ~25s end to end (~8 MiB/s, exactly the pacer's steady rate), and a 585 MB `--output type=tar` image export — unaffected by the pacer, since exports travel the unpaced direction — takes ~2.1s (~266 MiB/s); both logged zero stalls or link-recovery events. With the pacer's rate temporarily raised to 512 MiB/s for diagnosis, the same 200 MiB context moved in ~1s (~200 MiB/s) with no stalls either: the exec pipe itself sustains well over 100 MiB/s, so no non-exec bulk transport is implemented. Retuning the pacer's shipped default for the context-upload direction is a smaller, separate follow-up |
